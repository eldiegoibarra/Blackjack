...
let cards = [];
let dealerCards = []; // Store the dealer's cards
let sum = 0;
let dealerSum = 0; // Store the dealer's sum
...
let dealerCardsEl = document.getElementById("dealer-cards-el"); // Dealer's cards element
...
// Using addEventListener for the "Start Game" button
startGame.addEventListener("click", resetGame);

// Using addEventListener for the "New Card" button
...
// Using addEventListener for the "Stand" button
standGame.addEventListener("click", function(){
    if (isAlive && !hasBlackJack) {
        isAlive = false; // Player has chosen to "Stand"
...
function renderGame() {
    // Update the display and check for win/loss conditions
